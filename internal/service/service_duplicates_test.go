...
func TestMergeCanonicalProfileReplacesDefaultLabel(t *testing.T) {
	...
	canonical := model.Profile{
		...
		HomePath:  `C:\Users\Admin\.codex`,
		CreatedAt: now,
		UpdatedAt: now,
	}
	duplicate := model.Profile{
		ID:             "codex-ec61",
		Label:          "heohaneul-shtprunner-com",
		Tool:           model.ToolCodex,
		Provider:       model.ToolCodex,
		HomePath:       `C:\Users\Admin\.codex`,
		Email:          "heohaneul@shtprunner.com",
		AccountID:      "ec61",
		Plan:           "plus",
		AutoDiscovered: true,
	...
	if merged.Label != "heohaneul-shtprunner-com" {
		t.Fatalf("expected merged label to be updated, got %q", merged.Label)
	}
	if merged.Email != "heohaneul@shtprunner.com" {
